...
namespace NBSPI_INVENTORY_SYSTEM
{
    public partial class HMADD : Form
    {
        ...
        public HMADD(IT control, string item = "", int quantity = 0, string category = "", string brand = "", string model = "")
        {
            InitializeComponent();
        ...
        private void rjButton22_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(conn))
            {
                ...
                SqlCommand checkCmd = new SqlCommand("SELECT COUNT(*) FROM IT WHERE ITEM = @ITEM", con);
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO dbo.IT ([ID],[ITEM],[BRAND],[MODEL],[CATEGORY],[QUANTITY],[STATUS],[DATE]) Values (@ID,@ITEM,@BRAND,@MODEL,@CATEGORY,@QUANTITY,@STATUS,@DATE)", con);
